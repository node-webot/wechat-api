...
/**
 * 小程序发送模板消息
 * 详细细节: https://mp.weixin.qq.com/debug/wxadoc/dev/api/notice.html
 * 注:token获取和缓存方法和公众号一样 (缓存时请使用小程序APPID&APPSECRET获取小程序的access_token)
 *
 * Examples:
 * ```
 * var openid = '用户OPENID';
 * var tplID = '小程序模板id';
 * var url = '点击模板卡片后的跳转页面';
 * var emphasisKeyword = "keyword3.DATA";
 * var formId = 'form submit得到的formId或者支付是得到的prepay_id';(每个formId只能发送1次模板消息,详见文档)
 * var data = {
 *    "keyword1":{
 *      "value":"17012015374790837498A",
 *      "color":"#333333"
 *    },
 *    "keyword2": {
 *      "value":"13588889999",
 ...
 *    },
 *    "keyword4":{
 *      "value":"2017-01-20 15:38:13",
 *      "color":"#333333"
 *    }
 * };
 * api.xcxSendTemplate(openid, tplID, url, data,formId,emphasisKeyword,callback);
 * ```
 * Callback:
 *
 *正常时的返回JSON数据包示例：
 ...
 * }
 *
 * @param openid  用户的openid(必填)
 * @param templateId 模板ID(必填)
 * @param url  点击模板卡片后的跳转页面，仅限本小程序内的页面。支持带参数,（示例index?foo=bar）。该字段不填则模板无跳转。(非必填)
 * @param data  渲染模板的数据(必填)
 * @param formId  表单提交场景下，为 submit 事件带上的 formId；支付场景下，为本次支付的 prepay_id(必填)
 * @param emphasisKeyword  模板需要放大的关键词，不填则默认无放大(非必填)
 * @param callback 回调函数
 * @param callback2
 */
exports.xcxSendTemplate = function (openid, templateId, url, data, formId, emphasisKeyword, callback, callback2) {
  this.preRequest(this._xcxSendTemplate, arguments);
};

exports._xcxSendTemplate = function (openid, templateId, url, data, formId, emphasisKeyword, callback, callback2) {
  /*
   duplicated interface `function (openid, templateId, url, data, formId, emphasisKeyword, callback)`
   */
  var apiUrl = this.endpoint + '/cgi-bin/message/wxopen/template/send?access_token=' + this.token.accessToken;
