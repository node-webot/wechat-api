 ...
 * }
 *
 * @param params --openid(必填)  用户的openid
 * --templateId(必填) 模板ID
 * --url(非必填)  点击模板卡片后的跳转页面，仅限本小程序内的页面。支持带参数,（示例index?foo=bar）。该字段不填则模板无跳转。
 * --data(必填)  渲染模板的数据
 * --formId(必填)  表单提交场景下，为 submit 事件带上的 formId；支付场景下，为本次支付的 prepay_id
 * --emphasisKeyword(非必填)  模板需要放大的关键词，不填则默认无放大
 * @param callback 回调函数
 * @param callback2
 */
exports.xcxSendTemplate = function (params, callback, callback2) {
  this.preRequest(this._xcxSendTemplate, arguments);
};

exports._xcxSendTemplate = function (params, callback, callback2) {
  /*
  ...
  var template = {
    ...
    page: params.url || '',
    data: params.data,
    emphasis_keyword:params.emphasisKeyword || ''
  };
  this.request(apiUrl, postJSON(template), wrapper(callback));
};
