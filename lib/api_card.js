...
make(exports, 'getMembercard', function (info, callback) {
  var url = 'https://api.weixin.qq.com/card/membercard/userinfo/get?access_token=' + this.token.accessToken;
  this.request(url, postJSON(info), wrapper(callback));
});
...
make(exports, 'addConsumer', function (username, locationId, callback) {
  var url = 'http://api.weixin.qq.com/card/consumer/add?access_token=' + this.token.accessToken;
  var data = {
    'username': username,
 ...
 *
 * @name getMemberCardUserInfo
 * @param {Object} info 查询条件 {"card_id": abd5e78412e5d12ff,"code": 5566778811002233, "record_bonus" : 3000}
 * @param {Function} callback 回调函数
 */

make(exports, 'updateMemberCardUserInfo', function (info, callback) {
  var url = 'https://api.weixin.qq.com/card/membercard/updateuser?access_token=' + this.token.accessToken;
  var data = info;
  this.request(url, postJSON(data), wrapper(callback));
});
