...
API.patch  = function (functionName, apiUrl, override) {
  ...
  if (typeof apiUrl === 'function') {
    throw new Error('Not support self-implements function, yet  : '+ functionName);
  } else {
    if (API.prototype[functionName] || API.prototype['_' + functionName] ) {
      if (override !== true) {
        throw new Error('wechat-api already has a prototype named ['+ functionName + ']');
      } else {
        console.warn('wechat-api already has a prototype named ['+ functionName + ']');
        console.error('Overriding it with new function');
      }
    }
    util.make(API.prototype, functionName, function (info, callback) {
      ...
      var data = info;
      this.request(url, util.postJSON(data), wrapper(callback));
    });
  }
};
