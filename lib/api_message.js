 ...
 * ```
 * var miniprogrampage = {
 *  title: 'title', // 小程序卡片的标题，可选
 *  appid: 'appid', // 小程序的appid，要求小程序的appid需要与公众号有关联关系
 *  pagepath: 'pagepath', // 小程序的页面路径，跟app.json对齐，支持参数，比如pages/index/index?foo=bar
 *  thumb_media_id: 'thumb_media_id' // 小程序卡片图片的媒体ID
 * };
 * api.sendMiniProgramPage('openid', miniprogrampage, callback);
 * ```
 * Callback:
 *
 * - `err`, 调用失败时得到的异常
 ...
 *
 * @param {String} openid 用户的openid
 * @param {Object} miniprogrampage 小程序页面
 * @param {Function} callback 回调函数
 */
 ...
 * @param {Function} callback 回调函数
 */
 exports.sendMiniProgramPageFromCs = function (openid, cs_account, miniprogrampage, callback) {
  this.preRequest(this._sendMiniProgramPageFromCs, arguments);
};

/*!
 * 客服消息，发送小程序卡片消息的未封装版本
 */
exports._sendMiniProgramPageFromCs = function (openid ,cs_account, miniprogrampage, callback) {
  // {
  //  "touser":"OPENID",
  //  "msgtype":"miniprogrampage",
  //  "miniprogrampage": {
