...
/**
 * 多台服务器负载均衡时，ticketToken需要外部存储共享，则需要调用此regTicket来设置get和set的自定义方法
 * Examples:
 * ```
 * api.regTicket(getTicketToken, saveTicketToken);
 * function getTicketToken(callback) {
 *      if (ticketToken) return callback(null, ticketToken);
 *      settingModel.getItem({key:'weixin_ticketToken'}, function(err, setting) {
 *          if (err) return callback(err);
 *          if (setting && setting.value) ticketToken=setting.value;
 *          callback(null, ticketToken);
 *      });
 *  }
 * function saveTicketToken(_ticketToken, callback) {
 *      ticketToken=_ticketToken;
 *      settingModel.setItem({key:'weixin_ticketToken', value:ticketToken}, function(err, setting) {
 *          if (err) return callback(err);
 *          callback(null, ticketToken);
 *      });
 *  };
